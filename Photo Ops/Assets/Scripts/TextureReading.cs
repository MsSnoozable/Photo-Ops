...
public static class TextureReading
{
    ...
    public static (float damage, string reason)CheckEnemyPercentOnScreen(Texture2D photoResult, Color body, Color face) {
        ...
        bool isBottomPixelChecked = false, isTopPixelChecked = false;
        ...
        for (int i = 0; i < screen_pixels.Length && !isTopPixelChecked; i++)
        {
            if (screen_pixels[i] == face && !isBottomPixelChecked)
            {
            ...
            if (isBottomPixelChecked)
            {
                ...
                for (int j = screen_pixels.Length - 1; j > i && !isTopPixelChecked; j--)
                {
                    if (screen_pixels[j] == face && !isTopPixelChecked)
                    {
                        isTopPixelChecked = true;
                        if (j + photoResult.width > screen_pixels.Length - 1)
        ...
        //todo: if this changes from static to dynamic this can be removed 
